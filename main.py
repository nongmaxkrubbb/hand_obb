...
try:
    ...
    print("Segmenter not available (make sure selfie_segmenter.tflite exists)", e)
...
print("Opening Camera... Press 'q' on keyboard to exit.")
...
while True:
    ...
    if results.hand_landmarks:
        # Check gesture to change filter
        ...
        # Check pinch gesture thumb & index
        for hand_lms in results.hand_landmarks:
            ...
            # Backup filter change gesture (Thumb & Pinky)
            pinky = hand_lms[20]
        ...
        # 4 POINTS -> TILTED PORTAL (Polygon)
        if len(pts_portal) == 4:
            ...
            if bw > 0 and bh > 0:
                ...
                # --- GLOW PARTICLES AT PORTAL EDGES ---
                for i in range(4):
                    pt1 = poly_pts[i]
                    pt2 = poly_pts[(i+1)%4]
                    for _ in range(5):
    ...
    # Text Overlay
    cv2.putText(img, "Change Filter: Thumb & Pinky", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(img, f"Active Filter: {filters[current_filter]}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

    cv2.imshow('RETROLENS using Python', img)
